#
# PYro defaults configuration file
#   Chris Martin (c) 2015
#
# When PYro loads config files, it automatically looks for 'defaults.py' 
...
# PYro decides to load its configuration options.  PYro inspects the 
# locals() dictionary to see what (if any) variables were created.  If 
# the parameters are recognized, they are loaded into the pyro.config 
# dictionary.  This means that script files are just ordinary python code.
#
# PYro uses an unusual configuration loading scheme where no configuration 
...
#** Configuration files **
# This tells PYro where to find other configuration files. Keep in mind 
...
#   or, perhaps
#
#> config_file = '/etc/pyro.py'
#
#   also popular is
#
#> config_file = '~/.pyro/config.py'
#
...
#   or
#
#> dat_dir = ['/usr/share/pyro/data', '/home/bestuserever/.pyro/data']

# Should the PYro dat module print its load activity to stdout?
#
dat_verbose = False
...
# as whatever user loads PYro.
#
#> reg_dir = '~/.pyro/reg'
#
#   or
#
#> reg_dir = ['/usr/share/pyro/reg', '/home/bestuserever/.pyro/reg']

# Should the pyro.reg.regload() function print its activity to stdout?
reg_verbose = False
...
# Should a redundant class definition cause PYro to exit with an error?
